    ...
    namespace {
        void renderFontsPass(BaseSystem& baseSystem, std::vector<Entity>& prototypes, GLFWwindow* win, FontRenderPass pass) {
            ...
            for (const auto& ref : fontCtx.textInstances) {
                ...
                if (kDebugRelayoutTrackLabels && pass == FontRenderPass::SideButtons) {
                    if (inst->controlId.rfind("track_", 0) == 0 || inst->controlId.rfind("midi_track_", 0) == 0) {
                        float baseX = 200.0f;
                        if (inst->controlId.find("_clear") != std::string::npos) pos.x = baseX;
                        else if (inst->controlId.find("_input") != std::string::npos) pos.x = baseX + 60.0f;
                        else if (inst->controlId.find("_arm") != std::string::npos) pos.x = baseX + 120.0f;
                        else if (inst->controlId.find("_solo") != std::string::npos) pos.x = baseX + 180.0f;
                        else if (inst->controlId.find("_mute") != std::string::npos) pos.x = baseX + 240.0f;
                        else if (inst->controlId.find("_output") != std::string::npos) pos.x = baseX + 300.0f;
                    }
                }
