...
namespace RayTracedAudioSystemLogic {
    ...
    void ProcessRayTracedAudio(BaseSystem& baseSystem, std::vector<Entity>& prototypes, float dt, GLFWwindow* win) {
        ...
        if (baseSystem.voxelWorld && baseSystem.voxelWorld->enabled && baseSystem.registry) {
            int maxRadius = 0;
            for (int lod = 0; lod <= 6; ++lod) {
                std::string key = "voxelLod" + std::to_string(lod) + "Radius";
                auto it = baseSystem.registry->find(key);
                if (it == baseSystem.registry->end()) continue;
                if (!std::holds_alternative<std::string>(it->second)) continue;
                try {
                    int radius = std::stoi(std::get<std::string>(it->second));
                    if (radius > maxRadius) maxRadius = radius;
                } catch (...) {
                    continue;
                }
            }
            if (maxRadius > 0) maxDistance = static_cast<float>(maxRadius);
        }
