...
namespace UIScreenSystemLogic {
    ...
    namespace {
        ...
        EntityInstance* findTargetedComputer(BaseSystem& baseSystem,
            ...
            Entity& world = baseSystem.level->worlds[worldIndex];
            for (auto& inst : world.instances) {
                ...
                if (proto.name == "Computer") return &inst;
            }
            return nullptr;
        }
        ...
        void setComputerColor(BaseSystem& baseSystem, const glm::vec3& color) {
            ...
            if (ui.activeWorldIndex < 0 || ui.activeWorldIndex >= static_cast<int>(baseSystem.level->worlds.size())) return;
            Entity& world = baseSystem.level->worlds[ui.activeWorldIndex];
            for (auto& inst : world.instances) {
                if (inst.instanceID == ui.activeInstanceID) {
                    inst.color = color;
                    break;
                }
            }
    ...
    void UpdateUIScreen(BaseSystem& baseSystem, std::vector<Entity>& prototypes, float dt, GLFWwindow* win) {
        ...
        for (size_t wi = 0; wi < baseSystem.level->worlds.size(); ++wi) {
            auto& world = baseSystem.level->worlds[wi];
            for (auto& inst : world.instances) {
                if (inst.prototypeID < 0 || inst.prototypeID >= static_cast<int>(prototypes.size())) continue;
                if (prototypes[inst.prototypeID].name != "Computer") continue;
                if (ui.active && static_cast<int>(wi) == ui.activeWorldIndex && inst.instanceID == ui.activeInstanceID) continue;
                inst.color = darkGray;
            }
        }
        ...
        if (renderer.uiShader) {
            ...
            if (it != world.colorLibrary.end()) color = it->second;
            renderer.uiShader->setVec3("color", color);
            glBindVertexArray(renderer.uiVAO);
