...
uniform int leafBackfacesWhenInside;
uniform int leafDirectionalLightingEnabled;
uniform float leafDirectionalLightingIntensity;
...
uniform int grassAtlasShortBaseTile;
uniform int grassAtlasTallBaseTile;
uniform float time;
...
void main() {
    ...
    bool isGrassCover = (Alpha <= -13.5);
    bool isCavePot = (Alpha <= -9.5) && !isGrassCover;
    bool isSlopeFace = (Alpha <= -3.5) && (Alpha > -9.5);
    bool isSlopeCapA = (Alpha <= -3.5) && (Alpha > -4.5);
    bool isSlopeCapB = (Alpha <= -4.5) && (Alpha > -5.5);
    float outAlpha = (isSlopeFace || isCavePot || isGrassCover) ? 1.0 : max(Alpha, 0.0);
    ...
    // When canopy backfaces are enabled, only leaves should render as two-sided.
    if (leafBackfacesWhenInside == 1 && !gl_FrontFacing && !isLeaf) discard;
    ...
    if (isGrassCover && useAtlas) {
        ...
        int tileY = tilesPerCol - 1 - (TileIndex / tilesPerRow);
        vec2 base = vec2(tileX, tileY) * tileSizeUV;
        // Sample the block underneath using snapped 24x24 cell centers in world-space XZ.
        vec2 blockUv = fract(WorldPos.xz + vec2(0.5));
        vec2 snappedUv = (floor(blockUv * 24.0) + vec2(0.5)) / 24.0;
        vec2 atlasUv = base + snappedUv * tileSizeUV;
    ...
    if (blockDamageEnabled == 1
        ...
        && !isSlopeFace
        && !isTranslucentFace) {
        bool useDirectInstanceCell = isGrass || isFlower || isCavePot || isGrassCover;
        ivec3 cell = ivec3(0);
        if (useDirectInstanceCell) {
            // Foliage and grass-cover cards are centered in the owning voxel; this
            // avoids bleeding the erosion mask onto neighboring block faces.
            cell = ivec3(round(InstanceCell));
        } else {
            // Reconstruct the owning voxel cell for regular block faces.
            // Using WorldPos alone aliases neighboring cells on face boundaries;
            // using InstanceCell alone aliases face centers (+/-0.5). So we combine both.
            cell = ivec3(floor(WorldPos + vec3(0.5)));
            float halfStep = 0.5 * exp2(float(max(sectionLod, 0)));
            if (faceType == 0) {
                cell.x = int(round(InstanceCell.x - halfStep));
            } else if (faceType == 1) {
                cell.x = int(round(InstanceCell.x + halfStep));
            } else if (faceType == 2) {
                cell.y = int(round(InstanceCell.y - halfStep));
            } else if (faceType == 3) {
                cell.y = int(round(InstanceCell.y + halfStep));
            } else if (faceType == 4) {
                cell.z = int(round(InstanceCell.z - halfStep));
            } else if (faceType == 5) {
                cell.z = int(round(InstanceCell.z + halfStep));
            }
        }
    ...
    if (!isLeaf && !isGrass && !isFlower && !isCavePot && !isTranslucentFace) {
        float grid = 24.0;
        float line = 0.03;
    ...
    if (isGrass) {
        ...
        bool canUseGrassAtlas = (atlasEnabled == 1)
            && tilesPerRow > 0
            && tilesPerCol > 0
            && atlasTextureSize.x > 0.0
            && atlasTextureSize.y > 0.0;
        if (canUseGrassAtlas) {
            // Use runtime-selected grass atlas bands (conifer set by default), with legacy fallback.
            // If the prototype provides a tile index, treat it as the base of a 3-tile variant run.
            int shortBase = (grassAtlasShortBaseTile >= 0) ? grassAtlasShortBaseTile : 39;
            int tallBase = (grassAtlasTallBaseTile >= 0) ? grassAtlasTallBaseTile : 43;
            if (TileIndex >= 0) {
                if (isShortGrass) shortBase = TileIndex;
                else tallBase = TileIndex;
            }
            int atlasTile = (isShortGrass ? shortBase : tallBase) + variant;
            vec2 tileSizeUV = atlasTileSize / atlasTextureSize;
            int tileX = atlasTile % tilesPerRow;
            int tileY = tilesPerCol - 1 - (atlasTile / tilesPerRow);
            vec2 base = vec2(tileX, tileY) * tileSizeUV;
            vec2 atlasUv = base + uv * tileSizeUV;
            vec4 texel = texture(atlasTexture, atlasUv);
    ...
    if (isFlower) {
        ...
        float rad = length(p);
        ivec3 plantCell = ivec3(floor(WorldPos + vec3(0.5)));
        float quarterPicker = noise(vec2(
            float(plantCell.x) * 0.173 + float(plantCell.y) * 0.731,
            float(plantCell.z) * 0.293 + float(plantCell.y) * 0.121
        ));
        int quarterTurns = int(floor(quarterPicker * 4.0));
        quarterTurns = clamp(quarterTurns, 0, 3);
        float phase = noise(floor(WorldPos.xz * 0.75)) * 6.2831853 + float(quarterTurns) * 1.57079632679;
    ...
    if (isCavePot) {
        ...
        float diff = max(dot(norm, normalize(lightDir)), 0.0);
        vec3 lighting = ambientLight + diffuseLight * diff;
        FragColor = vec4(bc * lighting * AO, outAlpha);
        return;
    }
    ...
    if (isLeaf) {
        ...
        // Directional-off leaf mode keeps the shadow-tone color without directional side shading.
        vec3 lighting = ambientLight;
        if (leafDirectionalLightingEnabled == 1) {
            vec3 norm = normalize(Normal);
            float diff = max(dot(norm, normalize(lightDir)), 0.0);
            float directionalIntensity = clamp(leafDirectionalLightingIntensity, 0.0, 1.0);
            // Directional leaf lighting darkens from the dark base instead of brightening toward white.
            float shadowAmount = 1.0 - diff;
            float shadowMultiplier = mix(1.0, 1.0 - 0.6 * shadowAmount, directionalIntensity);
