...
void main() {
    ...
    bool isGrassCover = (aAlpha <= -13.5);
    bool isCavePot = (aAlpha <= -9.5) && !isGrassCover;
    bool isSlopeFace = (aAlpha <= -3.5) && (aAlpha > -9.5);
    ...
    bool isPlant = isGrass || isFlower || isCavePot;
    if (isPlant) {
        ...
        if (isGrass) {
            ...
            // Keep flower rotation pixel-aligned: 0/90/180/270 degree buckets.
            float randQuarter = floor(hash12(cell + vec2(17.4, 83.2)) * 4.0);
            plantAngle += randQuarter * 1.57079632679;
        }
    ...
    // Preserve the logical cell for per-block effects (damage masks, etc.).
    InstanceCell = aOffset;
    if (isPlant) {
    ...
    if (aAlpha < 0.0) {
        ...
        if (!isPlant && !isSlopeFace && !isGrassCover && sectionLod == 0) {
