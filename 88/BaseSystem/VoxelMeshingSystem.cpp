...
namespace VoxelMeshingSystemLogic {
    ...
    namespace {
        ...
        enum class NarrowShape : int { Default = 0, Stick = 1, StonePebble = 2, PetalPile = 3, GrassCover = 4, BlueprintMat = 5 };
        ...
        NarrowHalfExtents narrowHalfExtentsForShape(NarrowLogAxis axis, NarrowShape shape) {
            ...
            if (shape == NarrowShape::GrassCover || shape == NarrowShape::BlueprintMat) {
        ...
        bool BuildVoxelGreedyMesh(BaseSystem& baseSystem,
            ...
            auto sameKey = [&](const MaskCell& a, const MaskCell& b) {
                ...
                if (a.narrowShape == NarrowShape::GrassCover
                    ...
                    || b.narrowShape == NarrowShape::BlueprintMat) return false;
                    ...
                    for (int slice = 0; slice < sliceLen; ++slice) {
                        ...
                        for (int v = 0; v < vLen; ++v) {
                            for (int u = 0; u < uLen; ++u) {
                                ...
                                if (passType == 3 && cell.plantType == PlantType::Flower) {
                                    // Flowers are still procedural in shader space; grasses use atlas base tiles.
                                    tileIndex = -1;
                                }
                                int idx = v * uLen + u;
                                ...
                                float encodedAlpha = (passType == 3)
                                    ...
                                    : alpha;
                                if (passType != 3 && currentShape == NarrowShape::GrassCover) {
                        ...
                        for (int v = 0; v < vLen; ++v) {
                            for (int u = 0; u < uLen; ++u) {
                                ...
                                NarrowHalfExtents narrowExt = narrowHalfExtentsForShape(seed.narrowAxis, seed.narrowShape);
            ...
            const bool cullPlantsBeyondLod0 = false;
    ...
    void UpdateVoxelMeshing(BaseSystem& baseSystem, std::vector<Entity>& prototypes, float, GLFWwindow*) {
        ...
        const bool cullPlantsBeyondLod0 = false;
        const bool waterTopOnlyOutsideLod0 = ::RenderInitSystemLogic::getRegistryBool(baseSystem, "WaterTopOnlyOutsideLod0", true);
        const bool linkSurfaceRenderToFoliageReady =
            ::RenderInitSystemLogic::getRegistryBool(baseSystem, "LinkSurfaceRenderToFoliageReady", true);
        if (superChunkSize < 1) superChunkSize = 1;
        auto toRenderKey = [&](const VoxelSectionKey& key) {
            bool useSuperChunk = key.lod >= superChunkMinLod
                && key.lod <= superChunkMaxLod
        ...
        auto sectionBlockedByFoliageLink = [&](const VoxelSectionKey& key) {
            if (!linkSurfaceRenderToFoliageReady) return false;
            return TreeGenerationSystemLogic::IsSectionAwaitingFoliageFirstPass(key);
        };
        ...
        if (useVoxelGreedyAsync) {
            ...
            for (auto& result : results) {
                if (sectionBlockedByFoliageLink(result.renderKey)) {
                    ...
                    voxelGreedy.dirtySections.insert(result.renderKey);
                    continue;
                }
        ...
        for (const auto& key : voxelWorld.dirtySections) {
            ...
            if (sectionBlockedByFoliageLink(key)) {
                const VoxelSectionKey renderKey = toRenderKey(key);
                releaseGreedyChunkIfPresent(voxelGreedy, renderKey);
                voxelGreedy.renderBuffersDirty.insert(renderKey);
            }
            voxelGreedy.dirtySections.insert(key);
        }
