...
namespace RenderInitSystemLogic {
    ...
    void CleanupRenderer(BaseSystem& baseSystem, std::vector<Entity>& prototypes, float dt, GLFWwindow* win) {
        ...
        if (renderer.isometricMapCachedTexture) {
            glDeleteTextures(1, &renderer.isometricMapCachedTexture);
            renderer.isometricMapCachedTexture = 0;
        }
        renderer.isometricMapCachedWidth = 0;
        renderer.isometricMapCachedHeight = 0;
        if (renderer.topDownMapTexture) {
            glDeleteTextures(1, &renderer.topDownMapTexture);
            renderer.topDownMapTexture = 0;
        }
