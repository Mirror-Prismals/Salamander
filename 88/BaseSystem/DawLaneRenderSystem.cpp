...
namespace DawLaneRenderSystemLogic {
    ...
    void UpdateDawLaneRender(BaseSystem& baseSystem, std::vector<Entity>&, float, GLFWwindow* win) {
        ...
        if (daw.settingsMenuOpen) {
            ...
            if (daw.settingsTab == 0) {
                ...
                drawAudioField(settingsLayout.audioOutField,
                               ...
                               jackOnline && !daw.settingsAudioOutputDevices.empty());
                drawAudioField(settingsLayout.audioInField,
                               "Input Device",
                               inputLabel,
                               settingsLayout.audioInPrev,
                               settingsLayout.audioInNext,
                               jackOnline && !daw.settingsAudioInputDevices.empty());
                drawButton(settingsLayout.audioRefreshBtn, "Refresh", false, !jackOnline);
        ...
        if (baseSystem.daw) {
            laneAlpha = std::clamp(baseSystem.daw->activeThemeBackground.a, 0.0f, 1.0f);
        }
        glBlendColor(0.0f, 0.0f, 0.0f, laneAlpha);
