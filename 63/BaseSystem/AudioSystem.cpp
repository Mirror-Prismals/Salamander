...
int jack_process_callback(jack_nframes_t nframes, void* arg) {
    ...
    // De-interleave into JACK ports (ChucK)
    ...
    for (int ch = 0; ch < chuckChannels && ch < totalOutputs; ++ch) {
        auto* out = outBuffers[ch];
        if (!out) continue;
    ...
    if (audioContext->daw) {
        ...
        const int dawStart = audioContext->dawOutputStart;
        if (playing) {
            ...
            for (int t = 0; t < DawContext::kTrackCount; ++t) {
                ...
                int trackOutIndex = dawStart + t;
                int busOutIndex = dawStart + DawContext::kTrackCount + bus;
                jack_default_audio_sample_t* trackOut = (trackOutIndex >= 0 && trackOutIndex < totalOutputs) ? outBuffers[trackOutIndex] : nullptr;
                jack_default_audio_sample_t* busOut = (busOutIndex >= 0 && busOutIndex < totalOutputs) ? outBuffers[busOutIndex] : nullptr;

                const std::vector<float>& data = track.audio;
                for (jack_nframes_t i = 0; i < nframes; ++i) {
                    size_t idx = static_cast<size_t>(playhead + i);
                    float sample = (idx < data.size()) ? data[idx] : 0.0f;
                    if (trackOut) trackOut[i] += sample;
                    if (busOut) busOut[i] += sample;
                }
            }
    ...
    void InitializeAudio(BaseSystem& baseSystem, std::vector<Entity>& prototypes, float dt, GLFWwindow* win) {
        ...
        for (int ch = 0; ch < audio.jackOutputChannels; ++ch) {
            ...
            if (ch >= audio.dawOutputStart) {
                ...
                if (dawIndex < DawContext::kTrackCount) {
                    name = "daw_track_" + std::to_string(dawIndex + 1);
                } else if (dawIndex == DawContext::kTrackCount + 0) {
                    name = "daw_L";
                } else if (dawIndex == DawContext::kTrackCount + 1) {
                    name = "daw_S";
                } else if (dawIndex == DawContext::kTrackCount + 2) {
                    name = "daw_FF";
                } else if (dawIndex == DawContext::kTrackCount + 3) {
                    name = "daw_R";
                } else {
        ...
        if (const char** playbackPorts = jack_get_ports(audio.client, nullptr, JACK_DEFAULT_AUDIO_TYPE,
            ...
            for (size_t i = 0; i < audio.output_ports.size() && playbackPorts[i]; ++i) {
        ...
        if (const char** capturePorts = jack_get_ports(audio.client, nullptr, JACK_DEFAULT_AUDIO_TYPE,
            ...
            for (size_t i = 0; i < audio.input_ports.size() && capturePorts[i]; ++i) {
                if (!audio.input_ports[i]) continue;
                jack_connect(audio.client, capturePorts[i], jack_port_name(audio.input_ports[i]));
            }
