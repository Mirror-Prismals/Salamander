...
void Host::init() {
    ...
    if (worldEntity && worldGenAudicleProto && blockProto) {
        // 1. Place the Audio Visualizer Audicle outside the box
        Entity* vizProto = findProto("AudioVisualizer");
        if (vizProto) {
            glm::vec3 vizPos = {10, 5, 0}; // Move it in front of the window
        ...
        int boxSize = 5; // Increased box size
