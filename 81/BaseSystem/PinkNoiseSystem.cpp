...
namespace PinkNoiseSystemLogic {
    ...
    void ProcessPinkNoiseAudicle(BaseSystem& baseSystem, std::vector<Entity>& prototypes, float dt, GLFWwindow* win) {
        ...
        if (emitterInstance) {
            ...
            if (rtAudio.micSourceStates.count(emitterInstance->instanceID)) {
                micState = rtAudio.micSourceStates[emitterInstance->instanceID];
                micStateFound = true;
            }
        ...
        bool micActive = instance_found && rtAudio.micCaptureActive && micStateFound;
