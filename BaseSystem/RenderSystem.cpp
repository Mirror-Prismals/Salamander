...
namespace RenderSystemLogic {
    ...
    namespace {
        ...
        bool ShouldRenderChunkBlock(const Entity& proto, bool faceCullingInitialized) {
            ...
            bool culledByFaces = faceCullingInitialized && (proto.isOpaque || proto.name == "Water") && proto.name != "TransparentWave";
    ...
    void RenderScene(BaseSystem& baseSystem, std::vector<Entity>& prototypes, float dt, GLFWwindow* win) {
        ...
        bool greedyActive = GreedyMeshingEnabled(baseSystem) && baseSystem.greedy && baseSystem.greedy->initialized;
        ...
        for (size_t worldIndex = 0; worldIndex < level.worlds.size(); ++worldIndex) {
            ...
            if (useChunkRendering && baseSystem.chunk) {
                ...
                if (worldIndex < chunkCtx.worldHasNonChunkable.size() &&
                    !chunkCtx.worldHasNonChunkable[worldIndex]) {
                    continue;
                }
            }
        ...
        if (useChunkRendering) {
            ...
            bool faceInitialized = (baseSystem.face && baseSystem.face->initialized) || greedyActive;
            ...
            if (!chunkCtx.renderBuffersDirty.empty()) {
                ...
                size_t buildCount = 0;
                size_t totalChunkInstances = 0;
                for (const auto& idx : chunkCtx.renderBuffersDirty) {
                    auto it = chunkCtx.chunks.find(idx);
                    if (it != chunkCtx.chunks.end()) {
                        totalChunkInstances += it->second.positions.size();
                    }
                ...
                std::cout << "RenderSystem: rebuilt " << buildCount
                          << " chunk render buffer(s) ("
                          << totalChunkInstances << " instances) in "
        ...
        if (greedyActive && renderer.faceShader && renderer.faceVAO) {
            GreedyContext& greedyCtx = *baseSystem.greedy;
            std::array<std::vector<FaceInstanceRenderData>, 6> greedyChunkInstances;
            for (const auto& [chunkKey, greedyChunk] : greedyCtx.chunks) {
                if (!shouldRenderChunk(chunkKey)) continue;
                for (size_t i = 0; i < greedyChunk.positions.size(); ++i) {
                    int faceType = (i < greedyChunk.faceTypes.size()) ? greedyChunk.faceTypes[i] : -1;
                    if (faceType < 0 || faceType >= 6) continue;
                    glm::vec3 color = (i < greedyChunk.colors.size()) ? greedyChunk.colors[i] : glm::vec3(1.0f);
                    int tileIndex = (i < greedyChunk.tileIndices.size()) ? greedyChunk.tileIndices[i] : -1;
                    float alpha = (i < greedyChunk.alphas.size()) ? greedyChunk.alphas[i] : 1.0f;
                    glm::vec4 ao = (i < greedyChunk.ao.size()) ? greedyChunk.ao[i] : glm::vec4(1.0f);
                    glm::vec2 scale = (i < greedyChunk.scales.size()) ? greedyChunk.scales[i] : glm::vec2(1.0f);
                    glm::vec2 uvScale = (i < greedyChunk.uvScales.size()) ? greedyChunk.uvScales[i] : scale;
                    greedyChunkInstances[faceType].push_back({greedyChunk.positions[i], color, tileIndex, alpha, ao, scale, uvScale});
                }
            }

            std::array<std::vector<FaceInstanceRenderData>, 6> greedyOpaque;
            std::array<std::vector<FaceInstanceRenderData>, 6> greedyAlpha;
            for (int f = 0; f < 6; ++f) {
                for (const auto& inst : greedyChunkInstances[f]) {
                    if (inst.alpha < 0.999f) greedyAlpha[f].push_back(inst);
                    else greedyOpaque[f].push_back(inst);
                }
            }
            drawFaceBatches(greedyOpaque, true);
            drawFaceBatches(greedyAlpha, false);
        }

        if (!greedyActive && baseSystem.face && baseSystem.face->initialized && renderer.faceShader && renderer.faceVAO) {
            ...
            for (const auto& [chunkKey, faceChunk] : faceCtx.faces) {
                ...
                for (size_t i = 0; i < faceChunk.positions.size(); ++i) {
                    ...
                    faceChunkInstances[faceType].push_back({faceChunk.positions[i], color, tileIndex, alpha, ao, glm::vec2(1.0f), glm::vec2(1.0f)});
                }
            }
