...
namespace DawStateSystemLogic {
    ...
    namespace {
        ...
        void updateTransportButtonVisuals(BaseSystem& baseSystem, const DawContext& daw) {
            ...
            for (auto& world : baseSystem.level->worlds) {
                for (auto& inst : world.instances) {
                    if (inst.actionType != "DawTransport") continue;
                    bool toggled = false;
        ...
        void updateDawUILayout(BaseSystem& baseSystem, GLFWwindow* win) {
            if (!baseSystem.level || !win) return;
            ...
            for (auto& world : baseSystem.level->worlds) {
                for (auto& inst : world.instances) {
                    if (inst.actionType == "DawTrack" && inst.actionKey == "output") {
                        int trackIndex = -1;
                        int busIndex = -1;
                        if (!parseTrackAndBus(inst.actionValue, trackIndex, busIndex)) continue;
                    ...
                    if (inst.textType == "UIOnly" && inst.text.size() == 1 && inst.position.x > outputThreshold) {
                        int busIndex = -1;
    ...
    void UpdateDawState(BaseSystem& baseSystem, std::vector<Entity>& prototypes, float dt, GLFWwindow* win) {
        ...
        if (ui.active) {
            updateDawUILayout(baseSystem, win);
