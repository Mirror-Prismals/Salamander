...
struct PlayerContext { float cameraYaw=-90.0f; float cameraPitch=0.0f; glm::vec3 cameraPosition=glm::vec3(0.0f,1.0f,5.0f); float mouseOffsetX=0.0f; float mouseOffsetY=0.0f; float lastX=1920/2.0f; float lastY=1080/2.0f; bool firstMouse=true; glm::mat4 viewMatrix; glm::mat4 projectionMatrix; };
...
struct TesseractContext { std::vector<vec4> vertices4D; std::vector<unsigned int> indices; std::unique_ptr<Shader> shader; GLuint VAO = 0, VBO = 0, EBO = 0; float angleXW = 0.0f, angleYW = 0.0f, angleZW = 0.0f; float targetAngleXW = 0.0f, targetAngleYW = 0.0f, targetAngleZW = 0.0f; };
...
struct BaseSystem {
    ...
    std::unique_ptr<TesseractContext> tesseract;
};
