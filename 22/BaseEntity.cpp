...
struct Entity {
    ...
    bool isBlock = false;
    bool isWorld = false;
    std::string audicleType = "false"; // "false", "true", "gated"
    bool isStar = false;
    bool isTesseract = false;
    bool isVolume = false; // For procedural generation

    // Data Payloads
    glm::vec3 fillOrigin;
    glm::vec3 fillDimensions;
    std::string fillBlockType;
    std::string fillColor;
    int count = 1; // For spawning multiples, e.g., stars
...
void from_json(const nlohmann::json& j, Entity& e) {
    ...
    if (j.contains("isBlock") && j.at("isBlock").get<bool>()) {
        e.isBlock = true; e.isRenderable = true; e.isSolid = true;
    }
    ...
    if (j.contains("isTesseract")) j.at("isTesseract").get_to(e.isTesseract);
