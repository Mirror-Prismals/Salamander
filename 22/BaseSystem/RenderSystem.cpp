...
// The actual code for this function now lives in SkyboxSystem.cpp.
...
namespace RenderSystemLogic {
    ...
    void RenderScene(BaseSystem& baseSystem, std::vector<Entity>& prototypes, float dt, GLFWwindow* win) {
        ...
        if (level.worlds.empty() || level.activeWorldIndex >= level.worlds.size()) return;
        ...
        // Call the function from its new home
        ...
        for (const auto& instance : activeWorld.instances) {
