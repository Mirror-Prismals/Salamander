...
namespace FaceCullingSystemLogic {
    ...
    namespace {
        ...
        void appendFace(FaceChunkData& out, const glm::vec3& position, const glm::vec3& color, int faceType, int tileIndex) {
    ...
    void RebuildAllFaces(BaseSystem& baseSystem, std::vector<Entity>& prototypes) {
        ...
        for (const auto& [chunkIndex, chunk] : chunkCtx.chunks) {
            ...
            for (size_t i = 0; i < chunk.positions.size(); ++i) {
                ...
                if (!occupied.count(cell + glm::ivec3(1, 0, 0))) appendFace(out, pos + glm::vec3(0.5f, 0.0f, 0.0f), color, 0, faceTileIndexFor(proto, 0, baseSystem.world.get()));
                if (!occupied.count(cell + glm::ivec3(-1, 0, 0))) appendFace(out, pos + glm::vec3(-0.5f, 0.0f, 0.0f), color, 1, faceTileIndexFor(proto, 1, baseSystem.world.get()));
                if (!occupied.count(cell + glm::ivec3(0, 1, 0))) appendFace(out, pos + glm::vec3(0.0f, 0.5f, 0.0f), color, 2, faceTileIndexFor(proto, 2, baseSystem.world.get()));
                if (!occupied.count(cell + glm::ivec3(0, -1, 0))) appendFace(out, pos + glm::vec3(0.0f, -0.5f, 0.0f), color, 3, faceTileIndexFor(proto, 3, baseSystem.world.get()));
                if (!occupied.count(cell + glm::ivec3(0, 0, 1))) appendFace(out, pos + glm::vec3(0.0f, 0.0f, 0.5f), color, 4, faceTileIndexFor(proto, 4, baseSystem.world.get()));
                if (!occupied.count(cell + glm::ivec3(0, 0, -1))) appendFace(out, pos + glm::vec3(0.0f, 0.0f, -0.5f), color, 5, faceTileIndexFor(proto, 5, baseSystem.world.get()));
            }
