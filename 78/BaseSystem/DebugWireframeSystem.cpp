...
namespace DebugWireframeSystemLogic {
    ...
    void UpdateDebugWireframe(BaseSystem& baseSystem, std::vector<Entity>& prototypes, float dt, GLFWwindow* win) {
        ...
        if (renderer.faceShader && renderer.faceVAO && (greedyActive || (baseSystem.face && baseSystem.face->initialized))) {
            ...
            if (greedyActive) {
                ...
                for (const auto& [chunkKey, greedyChunk] : greedyCtx.chunks) {
                    ...
                    for (size_t i = 0; i < greedyChunk.positions.size(); ++i) {
                        ...
                        faceBatches[faceType].push_back({greedyChunk.positions[i], color, tileIndex, alpha, ao, scale, uvScale});
                        ++faceInstanceCount;
                    }
                }
            }

            if (!greedyActive && baseSystem.face && baseSystem.face->initialized) {
                ...
                for (const auto& [chunkKey, faceChunk] : faceCtx.faces) {
                    ...
                    for (size_t i = 0; i < faceChunk.positions.size(); ++i) {
                        ...
                        faceBatches[faceType].push_back({faceChunk.positions[i], color, tileIndex, alpha, ao, glm::vec2(1.0f), glm::vec2(1.0f)});
                        ++faceInstanceCount;
                    }
                }
            }
            ...
            drewFaceChunks = faceInstanceCount > 0;
        }
