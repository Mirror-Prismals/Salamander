...
namespace GreedyMeshingSystemLogic { void MarkChunkDirty(BaseSystem&, const ChunkKey&); }
...
namespace ChunkSystemLogic {
    ...
    void RebuildChunk(BaseSystem& baseSystem, std::vector<Entity>& prototypes, const ChunkKey& key) {
        ...
        if (baseSystem.greedy) {
            GreedyMeshingSystemLogic::MarkChunkDirty(baseSystem, key);
        }
    }
    ...
    void UpdateChunks(BaseSystem& baseSystem, std::vector<Entity>& prototypes, float dt, GLFWwindow* win) {
        ...
        for (const auto& key : toRemove) {
            ...
            if (baseSystem.greedy) {
                baseSystem.greedy->chunks.erase(key);
                baseSystem.greedy->dirtyChunks.erase(key);
            }
