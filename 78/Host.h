...
struct GreedyContext {
    std::unordered_map<ChunkKey, GreedyChunkData, ChunkKeyHash> chunks;
    std::unordered_set<ChunkKey, ChunkKeyHash> dirtyChunks;
    bool initialized = false;
};
...
struct MidiContext {
    static constexpr int kTrackCount = 1;
    int trackCount = kTrackCount;
    float sampleRate = 44100.0f;
    bool initialized = false;
    bool recordingActive = false;
    bool recordStopPending = false;
    std::array<MidiTrack, kTrackCount> tracks;
    std::vector<EntityInstance*> trackInstances;
    std::vector<EntityInstance*> trackLabelInstances;
    std::vector<EntityInstance*> outputLabelInstances;
    bool uiCacheBuilt = false;
...
struct BaseSystem {
    ...
    std::unique_ptr<GreedyContext> greedy;
...
namespace GreedyMeshingSystemLogic { void UpdateGreedyMeshes(BaseSystem&, std::vector<Entity>&, float, GLFWwindow*); void MarkChunkDirty(BaseSystem&, const ChunkKey&); }
...
namespace MidiStateSystemLogic { void UpdateMidiState(BaseSystem&, std::vector<Entity>&, float, GLFWwindow*); void CleanupMidiState(BaseSystem&, std::vector<Entity>&, float, GLFWwindow*); }
