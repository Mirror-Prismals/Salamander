...
namespace GroundCraftingSystemLogic {
    namespace {
        ...
        bool beginRitual(BaseSystem& baseSystem, std::vector<Entity>& prototypes) {
            ...
            int worldIndex = level.activeWorldIndex;
            if (player.hasBlockTarget
                && player.targetedWorldIndex >= 0
                && player.targetedWorldIndex < static_cast<int>(level.worlds.size())) {
                worldIndex = player.targetedWorldIndex;
            }
            if (worldIndex < 0 || worldIndex >= static_cast<int>(level.worlds.size())) return false;

            glm::ivec3 anchor = glm::ivec3(glm::round(player.cameraPosition + glm::vec3(0.0f, -1.0f, 0.0f)));
            if (player.hasBlockTarget) {
                anchor = glm::ivec3(glm::round(player.targetedBlockPosition));
            }

            glm::ivec3 center(0);
            glm::ivec2 downDir(0, 1);
            if (!findNearestPetalRingCenter(baseSystem, prototypes, worldIndex, anchor, player.cameraPosition, center, downDir)) {
                return false;
            }
            ...
            s.applied = false;
            s.timer = 0.0f;
            s.duration = glm::clamp(readRegistryFloat(baseSystem, "GroundCraftingRitualDuration", 0.52f), 0.12f, 2.5f);
            s.applyTime = glm::clamp(readRegistryFloat(baseSystem, "GroundCraftingRitualApplyTime", 0.26f), 0.01f, s.duration - 0.01f);
