...
namespace GemSystemLogic {
    namespace {
        ...
        bool ensureGemAtlasCacheLoaded(const BaseSystem& baseSystem) {
            ...
            cache.atlasPixels.assign(pixelCount, 0u);
            glBindTexture(GL_TEXTURE_2D, renderer.atlasTexture);
            glGetTexImage(GL_TEXTURE_2D, 0, GL_RGBA, GL_UNSIGNED_BYTE, cache.atlasPixels.data());
    ...
    void RenderGems(BaseSystem& baseSystem, std::vector<Entity>& prototypes, float dt, GLFWwindow* win) {
        ...
        auto appendGemMesh = [&](const GemDropState& drop, const glm::vec3& centerPos, float fade, float scaleMul) {
            ...
            for (const glm::ivec3& cell : *voxels) {
                const float centerY = static_cast<float>(cell.y) * (kMiniVoxelSize * gemScale);
            ...
            for (const glm::ivec3& cell : *voxels) {
                const glm::vec3 localCenter = glm::vec3(cell) * (kMiniVoxelSize * gemScale) + glm::vec3(0.0f, alignYOffset, 0.0f);
