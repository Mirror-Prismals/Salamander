...
namespace CollisionSystemLogic {
    ...
    void ResolveCollisions(BaseSystem& baseSystem, std::vector<Entity>& prototypes, float dt, GLFWwindow* win) {
        ...
        // Player collision capsule: 0.5 wide, 1.8 tall.
        glm::vec3 halfExtents(0.25f, 0.9f, 0.25f);
        ...
        if (baseSystem.gems) {
            for (const GemDropState& drop : baseSystem.gems->drops) {
                const glm::vec3 gemCenter(
                    std::round(drop.position.x),
                    drop.position.y,
                    std::round(drop.position.z)
                );
                blockAABBs.push_back({gemCenter - glm::vec3(0.499f), gemCenter + glm::vec3(0.499f)});
            }
        }
