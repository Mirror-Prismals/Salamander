...
namespace RenderSystemLogic {
    ...
    void RenderScene(BaseSystem& baseSystem, std::vector<Entity>& prototypes, float dt, GLFWwindow* win) {
        ...
        std::array<std::vector<InstanceData>, 6> faceInstances;
        ...
        for (const auto& worldProto : level.worlds) {
            for (const auto& instance : worldProto.instances) {
                ...
                if (proto.name == "Face_PosX") { faceInstances[0].push_back({instance.position, instance.color}); continue; }
                if (proto.name == "Face_NegX") { faceInstances[1].push_back({instance.position, instance.color}); continue; }
                if (proto.name == "Face_PosY") { faceInstances[2].push_back({instance.position, instance.color}); continue; }
                if (proto.name == "Face_NegY") { faceInstances[3].push_back({instance.position, instance.color}); continue; }
                if (proto.name == "Face_PosZ") { faceInstances[4].push_back({instance.position, instance.color}); continue; }
                if (proto.name == "Face_NegZ") { faceInstances[5].push_back({instance.position, instance.color}); continue; }
        ...
        auto isFaceCulledBlock = [&](const Entity& proto) {
            ...
            return baseSystem.face && baseSystem.face->initialized;
        };

        if (useChunkRendering) {
            ChunkContext& chunkCtx = *baseSystem.chunk;
            float maxChunkDist = static_cast<float>(chunkCtx.renderDistanceChunks) * static_cast<float>(std::max(chunkCtx.chunkSize.x, chunkCtx.chunkSize.z));
            auto shouldRenderChunk = [&](const glm::ivec3& chunkIndex) {
                if (chunkCtx.renderDistanceChunks <= 0) return true;
                glm::vec3 minCorner = glm::vec3(chunkIndex * chunkCtx.chunkSize);
                glm::vec3 center = minCorner + (glm::vec3(chunkCtx.chunkSize) * 0.5f);
                return glm::length(center - playerPos) <= maxChunkDist;
            };

            for (const auto& [chunkIndex, chunk] : chunkCtx.chunks) {
                if (!shouldRenderChunk(chunkIndex)) continue;
                for (size_t i = 0; i < chunk.positions.size(); ++i) {
                    ...
                    if (isFaceCulledBlock(proto)) continue;
                    RenderBehavior behavior = RenderBehavior::STATIC_DEFAULT;
                    if (proto.name == "Branch") behavior = RenderBehavior::STATIC_BRANCH;
                    else if (proto.name == "Water") behavior = RenderBehavior::ANIMATED_WATER;
                    else if (proto.name == "TransparentWave") behavior = RenderBehavior::ANIMATED_TRANSPARENT_WAVE;
                    ...
                    if (behavior == RenderBehavior::STATIC_BRANCH) {
                        branchInstances.push_back({chunk.positions[i], chunk.rotations[i], chunk.colors[i]});
                    } else {
                        behaviorInstances[static_cast<int>(behavior)].push_back({chunk.positions[i], chunk.colors[i]});
                    }
                }
            }
        }
        ...
        if (renderer.faceShader && renderer.faceVAO) {
            ...
            for (int faceType = 0; faceType < 6; ++faceType) {
                const auto& instances = faceInstances[faceType];
        ...
        if (baseSystem.face && baseSystem.face->initialized && renderer.faceShader && renderer.faceVAO) {
            ...
            std::array<std::vector<InstanceData>, 6> faceChunkInstances;
            for (const auto& [chunkIndex, faceChunk] : faceCtx.faces) {
                (void)chunkIndex;
                for (size_t i = 0; i < faceChunk.positions.size(); ++i) {
                    ...
                    faceChunkInstances[faceType].push_back({faceChunk.positions[i], color});
                }
            }
            ...
            glBindVertexArray(renderer.faceVAO);
            for (int faceType = 0; faceType < 6; ++faceType) {
                const auto& instances = faceChunkInstances[faceType];
                if (instances.empty()) continue;
                renderer.faceShader->setInt("faceType", faceType);
                glBindBuffer(GL_ARRAY_BUFFER, renderer.faceInstanceVBO);
                glBufferData(GL_ARRAY_BUFFER, instances.size() * sizeof(InstanceData), instances.data(), GL_DYNAMIC_DRAW);
