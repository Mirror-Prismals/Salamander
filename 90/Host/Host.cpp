...
void Host::registerSystemFunctions() {
    ...
    functionRegistry["UpdateIsometricMap"] = IsometricMapSystemLogic::UpdateIsometricMap;
...
void Host::init() {
    ...
    auto normalizeVoxelSectionSize = [](int requested, int fallback) {
        // Meshing/LOD code assumes a power-of-two section size.
    ...
    if (baseSystem.voxelWorld) {
        ...
        int maxLod = readRegistryInt("voxelMaxLod", baseSystem.voxelWorld->maxLod);
        if (readRegistryBool("voxelForceLod0Only", false)) {
            maxLod = 0;
        }
        if (maxLod < 0) maxLod = 0;
        while (maxLod > 0 && (sectionSize >> maxLod) <= 0) {
            --maxLod;
        }
        baseSystem.voxelWorld->sectionSize = sectionSize;
        baseSystem.voxelWorld->maxLod = maxLod;
    }
...
void Host::mainLoop() {
    if (!std::get<bool>(registry["Program"]) || !renderBackend || !window) { return; }
    ...
    while (!PlatformInput::WindowShouldClose(window)) {
        ...
        bool shaderReloadShortcutDown = isShaderReloadShortcutDown(window);
