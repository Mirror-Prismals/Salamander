...
namespace VoxelMeshUploadSystemLogic {
    namespace {
        ...
        void BuildVoxelGreedyRenderBuffers(IRenderBackend& renderBackend,
            ...
            std::array<std::vector<FaceInstanceRenderData>, 6> alphaWaterLikeInstances;
            for (size_t i = 0; i < chunk.positions.size(); ++i) {
                ...
                // Keep genuinely translucent tagged foliage/card faces in the alpha pass
                // (depth-write off) so semi-alpha texels do not self-occlude/sky-bleed.
                if (alpha < 0.0f) {
                    if (isWaterSlopeAlpha(alpha)) {
                        // Draw water-like faces last in the alpha pass so submerged foliage
                        // does not overdraw the water surface.
                        alphaWaterLikeInstances[faceType].push_back(inst);
                    } else if (isFoliageTaggedAlpha(alpha)) {
                        alphaInstances[faceType].push_back(inst);
                    } else {
                        opaqueInstances[faceType].push_back(inst);
                    }
                } else if (alpha < 0.999f) {
                    // Standard translucent water faces: keep in dedicated water-like bucket.
                    alphaWaterLikeInstances[faceType].push_back(inst);
                } else {
                    opaqueInstances[faceType].push_back(inst);
                }
            ...
            if (webgpuGreedyLogEnabled()) {
                ...
                if (logBudget > 0) {
                    std::cerr << "[VoxelGreedy][Buffers] chunkFaces=" << chunk.positions.size()
                              ...
                              << " alphaWater(0..5)="
                              << alphaWaterLikeInstances[0].size() << ","
                              << alphaWaterLikeInstances[1].size() << ","
                              << alphaWaterLikeInstances[2].size() << ","
                              << alphaWaterLikeInstances[3].size() << ","
                              << alphaWaterLikeInstances[4].size() << ","
                              << alphaWaterLikeInstances[5].size()
                              << std::endl;
                    --logBudget;
                }
            }
            ...
            for (int faceType = 0; faceType < 6; ++faceType) {
                ...
                std::vector<FaceInstanceRenderData> alphaOrdered;
                alphaOrdered.reserve(
                    alphaInstances[faceType].size()
                    + alphaWaterLikeInstances[faceType].size()
                );
                alphaOrdered.insert(
                    alphaOrdered.end(),
                    alphaInstances[faceType].begin(),
                    alphaInstances[faceType].end()
                );
                alphaOrdered.insert(
                    alphaOrdered.end(),
                    alphaWaterLikeInstances[faceType].begin(),
                    alphaWaterLikeInstances[faceType].end()
                );
                buffers.alphaCounts[faceType] = static_cast<int>(alphaOrdered.size());
                if (buffers.alphaCounts[faceType] > 0) {
                    ...
                    renderBackend.uploadArrayBufferData(
                        ...
                        alphaOrdered.data(),
                        alphaOrdered.size() * sizeof(FaceInstanceRenderData),
                        false
                    );
                    renderBackend.configureVertexArray(
