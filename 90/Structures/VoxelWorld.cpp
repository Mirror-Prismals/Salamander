...
void VoxelWorldContext::setBlockWorld(const glm::ivec3& worldPos, uint32_t id, uint32_t color) {
    ...
    for (int parentLod = 1; parentLod <= maxLod; ++parentLod) {
        ...
        if (parentIt == sections.end()) {
            ...
            parentSection.skyLight.assign(static_cast<size_t>(parentSize * parentSize * parentSize), defaultSkyLightLevel);
            parentSection.blockLight.assign(static_cast<size_t>(parentSize * parentSize * parentSize), static_cast<uint8_t>(0));
