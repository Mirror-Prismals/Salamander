...
int jack_process_callback(jack_nframes_t nframes, void* arg) {
    ...
    MidiTrack* midiTrack = midiContext ? &midiContext->tracks[0] : nullptr;
    ...
    if (audioContext->daw) {
        ...
        if (playing) {
            ...
            if (midiTrack && midiTrack->solo.load(std::memory_order_relaxed)) {
                anySolo = true;
            }
                ...
                if (vst3 && static_cast<size_t>(t) < vst3->audioTracks.size()) {
                    ...
                    if (chain.monoInput.size() >= static_cast<size_t>(nframes)) {
                        for (jack_nframes_t i = 0; i < nframes; ++i) {
                            ...
                            chain.monoInput[i] = (idx < data.size()) ? data[idx] : 0.0f;
                        }
            ...
            bool midiAllowed = false;
            if (midiTrack) {
                bool solo = midiTrack->solo.load(std::memory_order_relaxed);
                bool mute = midiTrack->mute.load(std::memory_order_relaxed);
                midiAllowed = (!anySolo && !mute) || (anySolo && solo);
                if (midiAllowed) {
                    int bus = midiTrack->outputBus.load(std::memory_order_relaxed);
                    jack_default_audio_sample_t* busBuffer = (bus >= 0 && bus < DawContext::kBusCount) ? busOut[bus] : nullptr;
                    if (busBuffer) {
                        const std::vector<float>& data = midiTrack->audio;
                        float gain = midiTrack->gain.load(std::memory_order_relaxed);
            ...
            if (midiTrack && daw.transportRecording.load(std::memory_order_relaxed)
                && midiTrack->recordEnabled.load(std::memory_order_relaxed)) {
                int midiNote = midiContext ? midiContext->activeNote.load(std::memory_order_relaxed) : -1;
                float midiVelocity = midiContext ? midiContext->activeVelocity.load(std::memory_order_relaxed) : 0.0f;
                ...
                if (!usedInstrument) {
                    ...
                    if (midiContext && midiNote >= 0 && midiVelocity > 0.0f) {
                ...
                if (recordSource && midiTrack->recordRing) {
                    size_t writeSpace = jack_ringbuffer_write_space(midiTrack->recordRing);
                    size_t framesToWrite = std::min<size_t>(nframes, writeSpace / sizeof(float));
                    if (framesToWrite > 0) {
                        jack_ringbuffer_write(midiTrack->recordRing,
            ...
            if (midiTrack) {
                ...
                midiTrack->meterLevel.store(meterValue, std::memory_order_relaxed);
            }
        ...
        if (!playing) {
            ...
            if (midiTrack) {
                midiTrack->meterLevel.store(0.0f, std::memory_order_relaxed);
            }
        }
