    ...
    namespace {
        ...
        bool parseTrackIndexFromKey(const std::string& key,
            ...
            try {
                ...
                if (oneBased) idx -= 1;
                if (idx < 0 || idx >= DawContext::kTrackCount) return false;
                outTrack = idx;
                return true;
            } catch (...) {
                return false;
        ...
        void loadTracksIfAvailable(DawContext& daw) {
            ...
            for (int i = 0; i < DawContext::kTrackCount; ++i) {
                ...
                if (loadWavMonoFloat(inPath.string(), data, rate)) {
                    ...
                    float colorRate = (rate > 0) ? static_cast<float>(rate) : daw.sampleRate;
                    rebuildWaveform(daw.tracks[i], colorRate);
                } else {
                    daw.tracks[i].waveformMin.clear();
                    daw.tracks[i].waveformMax.clear();
                    daw.tracks[i].waveformColor.clear();
        ...
        void updateTrackButtonVisuals(BaseSystem& baseSystem, DawContext& daw) {
            ...
            for (auto* instPtr : daw.trackInstances) {
                ...
                if (inst.actionKey == "arm") {
                    ...
                    int trackIndex = -1;
                    int busIndex = -1;
                    if (parseTrackAndBus(inst.actionValue, trackIndex, busIndex)) {
                        ...
                        trackIndex = parseTrackIndex(inst.actionValue);
                        if (trackIndex < 0) continue;
                        int currentBus = daw.tracks[trackIndex].outputBus.load(std::memory_order_relaxed);
        ...
        void updateDawUILayout(DawContext& daw, GLFWwindow* win) {
            ...
            for (auto* instPtr : daw.outputLabelInstances) {
                ...
                if (inst.textType == "VariableUI") {
                    int trackIndex = -1;
                    if (!parseTrackIndexFromKey(inst.textKey, "daw_out_", true, trackIndex)) continue;
                    inst.position.x = outputX[0];
                    continue;
                }
        ...
        void updateInputLabels(BaseSystem& baseSystem, DawContext& daw, AudioContext& audio) {
            ...
            for (int i = 0; i < DawContext::kTrackCount; ++i) {
                ...
                int busIndex = daw.tracks[i].outputBus.load(std::memory_order_relaxed);
            ...
            if (ui.pendingActionType == "DawTransport") {
                ...
                if (key == "arm") {
                    ...
                    if (parseTrackAndBus(ui.pendingActionValue, trackIndex, busIndex)) {
                        ...
                        trackIndex = parseTrackIndex(ui.pendingActionValue);
                        if (trackIndex >= 0) {
