...
void main(){
    ...
    float scaleX = mix(0.65, 0.45, 1.0 - y01);
    vec2 outerPos = vec2(p.x / scaleX - 0.35, (p.y + 0.2) * 0.9);
    float outer = length(outerPos);
    vec2 innerPos = vec2((p.x + 0.2) / scaleX, (p.y + 0.25) * 0.8);
    float inner = length(innerPos);
    float rim = smoothstep(1.02, 0.98, outer) * (1.0 - smoothstep(0.48, 0.5, inner));
    float fillMask = step(p.y, mix(-1.0, 1.0, clamp(fillAmount, 0.0, 1.0)) * 0.98);
    rim *= fillMask;
    if(rim < 0.05) discard;
    vec3 fillStart = vec3(0.1, 0.8, 0.2);
    vec3 fillEnd = vec3(1.0, 0.5, 0.0);
    vec3 color = mix(fillStart, fillEnd, y01);
    FragColor = vec4(color, 0.85 * rim);
}
