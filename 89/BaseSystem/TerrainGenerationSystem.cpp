...
namespace TerrainSystemLogic {
    ...
    namespace {
        ...
        bool GenerateExpanseSectionVoxel(BaseSystem& baseSystem,
            ...
            const float chalkVeinChance = glm::clamp(getRegistryFloat(baseSystem, "ChalkVeinChance", 0.22f), 0.0f, 1.0f);
            const float chalkReplaceChance = glm::clamp(getRegistryFloat(baseSystem, "ChalkReplaceChance", 0.72f), 0.0f, 1.0f);
            ...
            auto hasSideWaterAtDepth = [&](int worldXi, int worldYi, int worldZi) -> bool {
                static const std::array<glm::ivec2, 4> kNeighbors = {
                    glm::ivec2(1, 0),
                    glm::ivec2(-1, 0),
                    glm::ivec2(0, 1),
                    glm::ivec2(0, -1)
                };
                for (const glm::ivec2& dir : kNeighbors) {
                    const int nx = worldXi + dir.x;
                    const int nz = worldZi + dir.y;
                    float neighborHeight = 0.0f;
                    const bool neighborLand = ExpanseBiomeSystemLogic::SampleTerrain(
                        worldCtx,
                        static_cast<float>(nx),
                        static_cast<float>(nz),
                        neighborHeight
                    );
                    if (neighborLand) continue;
                    if (worldYi <= waterFloorY) continue;
                    if (worldYi > waterSurfaceY) continue;
                    return true;
                }
                return false;
            };
                ...
                int x = column - z * size;
                    ...
                    if (lod > 0) {
                        ...
                        if (!isLand) {
                            ...
                            trySetCell(floorDivInt(surfaceY, scale),
                                       (isBeach ? sandBeachProto->prototypeID : biomeSurfaceProto->prototypeID),
                    ...
                    for (int y = 0; y < size; ++y) {
                        ...
                        if (rangeContains(surfaceY)) {
                            bool placeChalkHere = false;
                            if (chalkColumnCandidate) {
                                const bool aboveAir = true; // surface cells are exposed to air by definition
                                const bool sideWater = hasSideWaterAtDepth(worldXi, surfaceY, worldZi);
                                if (aboveAir && sideWater) {
                                    ...
                                    placeChalkHere = chalkRoll <= chalkReplaceChance;
                                }
                            }

                            if (placeChalkHere && chalkProto) {
                                voxelWorld.setBlockLod(
                                    lod,
                                    lodCoord,
                                    chalkProto->prototypeID,
                                    packColor(chalkColor),
                                    false
                                );
                                wroteAny = true;

                                if (chalkStickSpawnPercent > 0 && rangeContains(surfaceY + 1)) {
                                    const int stickY = surfaceY + 1;
                                    const glm::ivec3 stickCoord(
                                        sectionCoord.x * size + x,
                                        stickY,
                                        sectionCoord.z * size + z
                                    );
                                    if (VoxelMeshInitSystemLogic::GetVoxelIdAtLod(voxelWorld, lod, stickCoord) == 0u) {
                                        const uint32_t stickSeed = hash3DInt(
                                            worldXi + chalkSeed * 11,
                                            stickY + chalkSeed * 13,
                                            worldZi - chalkSeed * 17
                                        );
                                        if (static_cast<int>((stickSeed >> 3u) % 100u) < chalkStickSpawnPercent) {
        ...
        void UpdateExpanseVoxelWorld(BaseSystem& baseSystem,
                    ...
                    if (lod == 0) {
                        ...
                        if (farLodSurfaceOnly) {
                            ...
                            const int waterFloorSectionY = floorDivInt(
                                static_cast<int>(std::floor(cfg.waterFloor)),
                                scale * size
                            );
            ...
            if (columnMode && rebuildDesired) {
                ...
                for (int lod = 0; lod <= maxLod; ++lod) {
                    ...
                    const int minY = (lod == 0) ? std::min(cfg.minY, -96) : cfg.minY;
