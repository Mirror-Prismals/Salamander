...
namespace VoxelMeshingSystemLogic {
    ...
    namespace {
        ...
        enum class NarrowShape : int { Default = 0, Stick = 1, StonePebble = 2, PetalPile = 3, GrassCover = 4, BlueprintMat = 5, ChalkDust = 6 };
        ...
        constexpr int kChalkTileEncodeStride = 512;
        constexpr int kChalkDustTileBase = 186;
        constexpr int kChalkDustTileMax = 191;
        ...
        bool decodeChalkDustSnapshot(uint32_t packedColor, int& outTileIndex, int& outQuarterTurns) {
            const int encoded = static_cast<int>((packedColor >> 24) & 0xffu);
            if (encoded <= 0) return false;
            const int raw = encoded - 1;
            const int variant = raw / 4;
            const int tile = kChalkDustTileBase + variant;
            if (tile < kChalkDustTileBase || tile > kChalkDustTileMax) return false;
            outTileIndex = tile;
            outQuarterTurns = raw & 3;
            return true;
        }
        ...
        bool BuildVoxelGreedyMesh(BaseSystem& baseSystem,
            ...
            auto sameKey = [&](const MaskCell& a, const MaskCell& b) {
                ...
                if (a.narrowShape == NarrowShape::GrassCover
                    ...
                    || b.narrowShape == NarrowShape::ChalkDust) return false;
                        ...
                        for (int v = 0; v < vLen; ++v) {
                            for (int u = 0; u < uLen; ++u) {
                                ...
                                if (currentShape == NarrowShape::ChalkDust && currentMount == NarrowMount::Floor) {
                                    ...
                                    if (decodeChalkDustSnapshot(cell.packedColor, chalkTile, chalkQuarterTurns)) {
