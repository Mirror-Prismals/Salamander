...
namespace DawLaneInputSystemLogic {
    namespace {
        ...
        void refreshAudioDeviceLists(DawContext& daw, AudioContext& audio) {
            ...
            buildDeviceOptions(queryJackPorts(audio.client, JackPortIsOutput | JackPortIsPhysical),
                               inputNames,
                               inputGroups);
            buildDeviceOptions(queryJackPorts(audio.client, JackPortIsInput | JackPortIsPhysical),
                               outputNames,
                               outputGroups);
            ...
            auto resolveSelection = [](const std::vector<std::string>& names,
                                       const std::string& selectedName) -> int {
                if (names.empty()) return -1;
                if (selectedName.empty()) return 0;
                for (int i = 0; i < static_cast<int>(names.size()); ++i) {
                    if (names[static_cast<size_t>(i)] == selectedName) {
                        return i;
                    }
                }
                return 0;
            ...
            daw.settingsSelectedAudioInput = resolveSelection(daw.settingsAudioInputDevices,
                                                              daw.settingsAudioInputName);
            daw.settingsSelectedAudioOutput = resolveSelection(daw.settingsAudioOutputDevices,
            ...
            if (daw.settingsSelectedAudioInput >= 0
                ...
                daw.settingsAudioInputName =
                    daw.settingsAudioInputDevices[static_cast<size_t>(daw.settingsSelectedAudioInput)];
            }
            if (daw.settingsSelectedAudioOutput >= 0
                && daw.settingsSelectedAudioOutput < static_cast<int>(daw.settingsAudioOutputDevices.size())) {
                daw.settingsAudioOutputName =
                    daw.settingsAudioOutputDevices[static_cast<size_t>(daw.settingsSelectedAudioOutput)];
            }
        }
    ...
    void UpdateDawLaneInput(BaseSystem& baseSystem, std::vector<Entity>& prototypes, float dt, PlatformWindowHandle win) {
        ...
        if (daw.settingsMenuOpen) {
            ...
            auto refreshAudioSettings = [&]() {
                ...
                refreshAudioDeviceLists(daw, *baseSystem.audio);
            };
