...
void main(){
    ...
    vec3 backgroundColor = vec3(0.08, 0.08, 0.08);
    vec3 fillStart = vec3(0.1, 0.8, 0.2);
    vec3 fillEnd = vec3(1.0, 0.5, 0.0);
    if(ready) fillEnd = vec3(1.0, 0.9, 0.3);

    if(vUV.x < border || vUV.x > 1.0 - border || vUV.y < border || vUV.y > 1.0 - border){
        FragColor = vec4(borderColor, 0.9);
        return;
    }

    if(vUV.x <= fillAmount){
        float t = clamp(vUV.x / max(fillAmount, 0.0001), 0.0, 1.0);
        vec3 fillColor = mix(fillStart, fillEnd, t);
        FragColor = vec4(fillColor, 0.85);
    } else {
        FragColor = vec4(backgroundColor, 0.4);
    }
}
