...
void Host::registerSystemFunctions() {
    ...
    // Player Control System functions
...
void Host::init() {
    ...
    const std::vector<std::string> entityFiles = { "Entities/World.json", "Entities/Tesseract.json" };
    ...
    if (worldEntity && tesseractProto) {
        worldEntity->instances.push_back(HostLogic::CreateInstance(baseSystem, tesseractProto->prototypeID, {0, 5, 0}, {1,1,1}));
    }
