    ...
    void RenderScene(BaseSystem& baseSystem, std::vector<Entity>& prototypes, float dt, GLFWwindow* win) {
        ...
        for (size_t worldIndex = 0; worldIndex < level.worlds.size(); ++worldIndex) {
            ...
            for (const auto& instance : worldProto.instances) {
                ...
                if (proto.name == "Face_PosX") { faceInstances[0].push_back({instance.position, instance.color, -1, 1.0f, glm::vec4(1.0f)}); continue; }
                if (proto.name == "Face_NegX") { faceInstances[1].push_back({instance.position, instance.color, -1, 1.0f, glm::vec4(1.0f)}); continue; }
                if (proto.name == "Face_PosY") { faceInstances[2].push_back({instance.position, instance.color, -1, 1.0f, glm::vec4(1.0f)}); continue; }
                if (proto.name == "Face_NegY") { faceInstances[3].push_back({instance.position, instance.color, -1, 1.0f, glm::vec4(1.0f)}); continue; }
                if (proto.name == "Face_PosZ") { faceInstances[4].push_back({instance.position, instance.color, -1, 1.0f, glm::vec4(1.0f)}); continue; }
                if (proto.name == "Face_NegZ") { faceInstances[5].push_back({instance.position, instance.color, -1, 1.0f, glm::vec4(1.0f)}); continue; }
        ...
        if (baseSystem.face && baseSystem.face->initialized && renderer.faceShader && renderer.faceVAO) {
            ...
            for (const auto& [chunkKey, faceChunk] : faceCtx.faces) {
                ...
                for (size_t i = 0; i < faceChunk.positions.size(); ++i) {
                    ...
                    faceChunkInstances[faceType].push_back({faceChunk.positions[i], color, tileIndex, alpha, ao});
                }
            }
