...
namespace BlockSelectionSystemLogic {
    ...
    struct WorldBlockCache {
        ...
        bool usesChunkData = false;
        glm::ivec3 lastCameraChunk{std::numeric_limits<int>::max()};
        int lastRadius = -1;
    };
    ...
    void BuildCacheFromChunks(WorldBlockCache& cache,
                              int worldIndex,
                              const ChunkContext& chunkCtx,
                              const Entity& world,
                              const std::vector<Entity>& prototypes) {
        cache.blocks.clear();
        cache.cellLookup.clear();
        cache.positionLookup.clear();
        cache.usesChunkData = true;

        for (const auto& [chunkKey, chunk] : chunkCtx.chunks) {
            if (chunkKey.worldIndex != worldIndex) continue;
            for (size_t i = 0; i < chunk.positions.size(); ++i) {
                ...
                if (!proto.isChunkable) continue;
                BlockEntry entry;
                entry.center = chunk.positions[i];
                entry.min = chunk.positions[i] - glm::vec3(0.5f);
                entry.max = chunk.positions[i] + glm::vec3(0.5f);
                entry.worldIndex = worldIndex;
                entry.active = true;
                entry.dampingFactor = proto.dampingFactor;
                AddEntry(cache, entry);
    ...
    void BuildCacheFromChunksLocal(WorldBlockCache& cache,
                                   int worldIndex,
                                   const ChunkContext& chunkCtx,
                                   const Entity& world,
                                   const std::vector<Entity>& prototypes,
        ...
        for (const auto& [chunkKey, chunk] : chunkCtx.chunks) {
            if (chunkKey.worldIndex != worldIndex) continue;
            glm::ivec3 delta = chunkKey.chunkIndex - cameraChunk;
            if (std::abs(delta.x) > radius || std::abs(delta.y) > radius || std::abs(delta.z) > radius) continue;
            for (size_t i = 0; i < chunk.positions.size(); ++i) {
                if (i >= chunk.prototypeIDs.size()) continue;
                int protoID = chunk.prototypeIDs[i];
                if (protoID < 0 || protoID >= static_cast<int>(prototypes.size())) continue;
    ...
    void EnsureCacheBuilt(int worldIndex, BaseSystem& baseSystem, const std::vector<Entity>& prototypes) {
        ...
        bool chunkReady = baseSystem.chunk && baseSystem.chunk->initialized;
        if (!cache.initialized || (chunkReady && !cache.usesChunkData)) {
            if (chunkReady) {
                BuildCacheFromChunks(cache, worldIndex, *baseSystem.chunk, baseSystem.level->worlds[worldIndex], prototypes);
            } else {
    ...
    bool EnsureLocalCaches(BaseSystem& baseSystem,
                           ...
                           int radius) {
        if (!baseSystem.level || !baseSystem.chunk || !baseSystem.chunk->initialized) return false;
        if (radius < 0) radius = 0;
        const ChunkContext& chunkCtx = *baseSystem.chunk;

        auto chunkIndexFromPosition = [](const glm::vec3& position, const glm::ivec3& chunkSize) {
            auto floorDivInt = [](int value, int divisor) {
        ...
        glm::ivec3 cameraChunk = chunkIndexFromPosition(cameraPosition, chunkCtx.chunkSize);
        if (g_worldCaches.size() < baseSystem.level->worlds.size()) {
            g_worldCaches.resize(baseSystem.level->worlds.size());
        }
        for (size_t i = 0; i < baseSystem.level->worlds.size(); ++i) {
            WorldBlockCache& cache = g_worldCaches[i];
            bool needsRebuild = !cache.initialized ||
                !cache.usesChunkData ||
                cache.lastCameraChunk != cameraChunk ||
                cache.lastRadius < radius;
            if (needsRebuild) {
                BuildCacheFromChunksLocal(cache,
                                          static_cast<int>(i),
                                          chunkCtx,
                                          baseSystem.level->worlds[i],
                                          prototypes,
                                          cameraChunk,
                                          radius);
                cache.lastCameraChunk = cameraChunk;
    ...
    void UpdateBlockSelection(BaseSystem& baseSystem, std::vector<Entity>& prototypes, float dt, GLFWwindow* win) {
        ...
        if (!EnsureLocalCaches(baseSystem, prototypes, player.cameraPosition, kSelectionCacheRadius)) {
        ...
        int hitWorld = -1;
        if (RaycastBlocks(g_worldCaches, player.cameraPosition, dir, hitCell, hitCenter, hitNormal, hitWorld)) {
