...
namespace RayTracedAudioSystemLogic {
    ...
    const EntityInstance* getInstanceAt(const glm::ivec3& pos, const Entity& worldEntity) {
    ...
    void ProcessRayTracedAudio(BaseSystem& baseSystem, std::vector<Entity>& prototypes, float dt, GLFWwindow* win) {
        ...
        int activeIndex = level.activeWorldIndex;
        if (activeIndex < 0 || activeIndex >= static_cast<int>(level.worlds.size())) activeIndex = 0;
        Entity& activeWorld = level.worlds[activeIndex];

        // --- Ray Marching for each sound source ---
        for (const auto& sourceInstance : activeWorld.instances) {
            if (sourceInstance.prototypeID == visualizerProtoID) {
                ...
                while (distanceTraveled < maxDistance) {
                    ...
                    const EntityInstance* hitInstance = getInstanceAt(currentVoxel, activeWorld);
                    if (hitInstance) {
