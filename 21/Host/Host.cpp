...
void Host::init() {
    loadRegistry();
    if (programStatus != "Installed") { std::cerr << "FATAL: Program is not 'Installed' in registry.json. Halting." << std::endl; return; }
    ...
    const std::vector<std::string> entityFiles = { "Entities/World.json", "Entities/Tesseract.json", "Entities/Star.json" };
    ...
    Entity* tesseractProto = findProto("Tesseract");

    // --- The Tesseract's world position is the origin ---
    if (worldEntity && tesseractProto) {
        worldEntity->instances.push_back(HostLogic::CreateInstance(baseSystem, tesseractProto->prototypeID, {0, 0, 0}, {1,1,1}));
    }

    // --- The Player's starting position is the origin ---
    if (baseSystem.player) {
        baseSystem.player->cameraPosition = glm::vec3(0.0f, 0.0f, 0.0f);
    }
...
void Host::mainLoop() {
    if (programStatus != "Installed") { while (window && !glfwWindowShouldClose(window)) { glfwPollEvents(); glClearColor(0.1f, 0.0f, 0.0f, 1.0f); glClear(GL_COLOR_BUFFER_BIT); glfwSwapBuffers(window); } return; }
