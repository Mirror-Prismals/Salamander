// ======================================================================
// prismals_entity_system_final_v2.cpp
// A single-file implementation of the user's definitive Entity-System design.
// This version corrects all compiler errors from the previous attempt.
// ======================================================================
...
struct EntityInstance; // Forward declare
...
struct Entity {
    ...
    int blockType = -1; // -1 indicates not a standard block
    
    bool isWorld = false;
    std::vector<EntityInstance> instances; // Only used if isWorld is true
...
// --- Instance System ---
// The only system that can create new instances.
...
public:
    ...
    void update(std::vector<Entity>& prototypes, BaseSystem& baseSystem, float deltaTime, GLFWwindow* window) override {
        // This system doesn't do anything in the main loop, it's called on demand.
    }
};
    ...
    void init() {
        ...
        void main(){
            ...
            if(blockType==3||blockType==7||blockType==9||blockType==17){
                vec2 f=fract(TexCoord*grid); // Use the main grid size for leaves too
            // --- END OF FIX ---
...
private:
    void init() {
        ...
        #endif
        window = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "Prismals Entity-System Final", NULL, NULL);
        ...
        for (int i = 0; i < NUM_BLOCK_PROTOTYPES; ++i) {
            Entity p; p.prototypeID = i; p.isRenderable = true; p.blockType = i;
            entityPrototypes.push_back(p);
        }
        Entity worldProto; worldProto.prototypeID = entityPrototypes.size(); worldProto.isWorld = true;
        entityPrototypes.push_back(worldProto);
        
        Entity starProto; starProto.prototypeID = entityPrototypes.size(); starProto.isStar = true;
        entityPrototypes.push_back(starProto);

        // Create instances using an InstanceSystem
        InstanceSystem instanceSystem;
        
        // Find the world entity prototype to add instances to it
        for (auto& proto : entityPrototypes) {
            if (proto.isWorld) {
                ...
                for (int i = 0; i < NUM_BLOCK_PROTOTYPES; ++i) {
                    ...
                    proto.instances.push_back(instanceSystem.createInstance(i, pos));
                }
                for (int i = 0; i < NUM_STARS; i++) {
                    ...
                    proto.instances.push_back(instanceSystem.createInstance(starProto.prototypeID, pos));
                }
                break;
            }
        }
