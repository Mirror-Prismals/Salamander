...
void Host::PopulateWorldsFromLevel() {
    const std::vector<std::string> entityFiles = { 
        ...
        "Entities/Tesseract.json", "Entities/World.json", "Entities/DebugWorldGenerator.json",
        ...
        if (!worldProto.isVolume && !worldProto.instances.empty()) {
            ...
            for (const auto& instTemplate : templates) {
                ...
                for (int i = 0; i < count; ++i) {
                    ...
                    processedInstances.push_back(HostLogic::CreateInstance(baseSystem, entityProto->prototypeID, pos, {1,1,1}));
                }
            }
