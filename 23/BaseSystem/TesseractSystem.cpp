...
namespace TesseractSystemLogic {
    ...
    void RenderTesseract(BaseSystem& baseSystem, std::vector<Entity>& prototypes, float dt, GLFWwindow* win) {
        ...
        // Check if a tesseract prototype exists at all. If so, we render it.
        bool tesseractProtoExists = false;
        for(const auto& proto : prototypes) {
            if(proto.isTesseract) {
                tesseractProtoExists = true;
                break;
            }
        }
        if(!tesseractProtoExists) return;
