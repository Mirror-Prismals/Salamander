...
namespace PinkNoiseSystemLogic {
    float generate_pink_noise_sample(AudioContext*);
}
...
int jack_process_callback(jack_nframes_t nframes, void* arg) {
    ...
    if (active_gens > 0) {
        // If any generator is active, call it to fill the buffer.
        // For now, we only have one generator (PinkNoise).
        for (jack_nframes_t i = 0; i < nframes; i++) {
            float sample = PinkNoiseSystemLogic::generate_pink_noise_sample(audioContext);
            float final_sample = sample * audioContext->output_gain;
            out_buffer[i] = final_sample;
            
            // Write to ring buffer for visualization
            // Only write one sample per block to keep visualization simple and responsive
            if (i == 0) { 
        ...
        // Output silence if no generators are active
