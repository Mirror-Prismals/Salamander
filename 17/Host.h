...
struct AudioContext {
    ...
    float output_gain = 0.8f; // Master volume

    std::mutex audio_state_mutex;
    int active_generators = 0;

    // Pink noise state lives here because it's tied to the audio thread's sample-by-sample generation
...
// Struct to hold all calculated audio path properties
struct AudioSourceState {
    bool isOccluded = false;
    float distanceGain = 1.0f;
};

// Context to store the results of the ray tracing
struct RayTracedAudioContext {
    std::map<int, AudioSourceState> sourceStates; // Key: instanceID
};
...
namespace RayTracedAudioSystemLogic {
    void ProcessRayTracedAudio(BaseSystem&, std::vector<Entity>&, float, GLFWwindow*);
}
